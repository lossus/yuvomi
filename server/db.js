...
const MIGRATIONS = [
  ...
  {
    version: 89,
    description: 'Core pantry locations, stock lots, and immutable inventory movements',
    up: `
      ...
      CREATE INDEX IF NOT EXISTS idx_pantry_items_lookup
        ON pantry_items(name COLLATE NOCASE, category, location_id, expiry_date)
        WHERE deleted_at IS NULL;
      CREATE INDEX IF NOT EXISTS idx_pantry_items_low_stock
        ON pantry_items(minimum_amount, amount)
        WHERE deleted_at IS NULL AND minimum_amount IS NOT NULL;
  ...
  {
    version: 91,
    description: 'Meal cooking events, ingredient allocation snapshots, and inventory provenance',
    up: `
      ...
      CREATE TABLE IF NOT EXISTS meal_cooking_ingredients (
        ...
        created_at               TEXT    NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')),
        CHECK((amount_snapshot IS NULL AND unit_snapshot IS NULL) OR
              (amount_snapshot IS NOT NULL AND amount_snapshot > 0 AND unit_snapshot IN ('g', 'kg', 'ml', 'l')))
      );
      ...
      ALTER TABLE inventory_movements
        ADD COLUMN cooking_event_id INTEGER REFERENCES meal_cooking_events(id) ON DELETE SET NULL;

      CREATE UNIQUE INDEX IF NOT EXISTS idx_meal_cooking_events_active
        ON meal_cooking_events(meal_id)
        WHERE status = 'confirmed' AND meal_id IS NOT NULL;
      CREATE INDEX IF NOT EXISTS idx_meal_cooking_events_meal_created
        ON meal_cooking_events(meal_id, created_at DESC, id DESC);
      CREATE INDEX IF NOT EXISTS idx_meal_cooking_ingredients_event
        ON meal_cooking_ingredients(cooking_event_id, id);
      CREATE INDEX IF NOT EXISTS idx_meal_cooking_allocations_ingredient
        ON meal_cooking_allocations(cooking_ingredient_id, id);
      CREATE INDEX IF NOT EXISTS idx_inventory_movements_cooking_event
        ON inventory_movements(cooking_event_id, created_at DESC, id DESC)
