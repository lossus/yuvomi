...
- `public/components/datepicker.js`: existing micro-calendar implementation to reuse. A native or free-text date input should not be introduced.
...
The `shopping_import` block is optional. Omitting it, or sending `{ "enabled": false }`, preserves the previous behavior. If `enabled` is true, `list_id` is required and must reference an existing list; the backend does not silently choose a target. The UI preselects the first ordered list so the visible default remains explicit in the request. The response continues returning the created meal in `data` and additively includes `shopping_import: { enabled, list_id, transferred }` for an activated import.
...
- the UI preselects the lowest `sort_order`, while the server requires and uses the explicit target;
...
- existing manual transfer endpoints continue using `on_shopping_list` to avoid linking the same concrete meal ingredient twice; duplicate Meal-Create request idempotency requires a separate request-key design and is not invented here;
